...
// ============================================================================
// TEMPORARY SIGN-UP BLOCK -- urgent, see chat 2026-07-12. Blocks new
// sign-ups on web while proper trial/subscription entitlement gating is
...
// guard is untouched) -- this only stops the signup form from rendering.
//
...
function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // auth.currentUser is already set synchronously at this point, even
      // though the auth-context's onAuthStateChanged hasn't necessarily
      // caught up yet -- the dashboard guard checks currentUser directly
      // too, so navigating immediately here is safe.
        ...
        <section className="rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-8">
          ...
          <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl border border-border bg-background px-4 text-sm outline-none transition-colors placeholder:text-muted-foreground focus:border-primary"
              />
            </div>
            <div className="space-y-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="inline-flex h-11 w-full items-center justify-center rounded-full bg-primary px-5 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90 disabled:opacity-60"
            >
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <button
              ...
              className="flex h-11 items-center justify-center gap-2.5 rounded-xl border border-border bg-white text-sm font-medium text-foreground shadow-sm transition-all hover:bg-black/[0.02] hover:shadow-md disabled:opacity-60"
            >
              <GoogleIcon />
              Google
            </button>
            <button
              ...
              className="flex h-11 items-center justify-center gap-2.5 rounded-xl border border-border bg-white text-sm font-medium text-foreground shadow-sm transition-all disabled:cursor-not-allowed disabled:opacity-40"
            >
              <AppleIcon />
              Apple
            </button>
