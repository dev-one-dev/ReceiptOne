...
import { pageSEO, breadcrumbJsonLd, HREFLANG_US_CA } from "@/lib/seo";
import { articleJsonLd, type ContentBlock, type Article } from "@/lib/articles";
import { getHelpCenterArticle, getHelpCenterRelated } from "@/lib/help-center";
import { US_ARTICLES } from "@/lib/articles.us";

export const Route = (createFileRoute as any)("/us/articles/$slug")({
  head: ({ params }: { params: { slug: string } }) => {
    const article = getHelpCenterArticle(params.slug, US_ARTICLES);

    if (!article) {
      return { meta: [{ title: "Article Not Found | ReceiptOne" }], links: [] };
    }

    const seo = pageSEO({
      path: `/us/articles/${article.slug}`,
    ...
    const breadcrumb = breadcrumbJsonLd([
      ...
      { name: article.title, path: `/us/articles/${article.slug}` },
    ]);
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto flex min-h-[60vh] max-w-[760px] flex-col items-center justify-center px-4 py-24 text-center sm:px-6">
        ...
        <h1 className="mt-3 text-black">Article not found</h1>
        <p className="mt-4 font-sans text-body text-black/55">
...
function renderBlock(block: ContentBlock, index: number) {
  switch (block.type) {
    case "p":
      return (
        <p key={index} className="mb-5 font-sans text-body text-black/70">
          {block.text}
        </p>
      );
    case "h2":
      return (
        <h2 key={index} className="mb-4 mt-10 text-h3 text-black">
          {block.text}
        </h2>
      );
    case "ul":
        ...
        <ul key={index} className="mb-5 space-y-3">
          {block.items.map((item, i) => (
            <li key={i} className="flex gap-3">
              ...
              <span className="font-sans text-body text-black/70">{item}</span>
            </li>
          ))}
        </ul>
      );
      ...
      return (
        ...
        >
          <div className="flex gap-0">
            ...
            <p className="p-6 font-sans text-body text-black/70">{block.text}</p>
          </div>
        </div>
      );
    default:
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-auto flex items-center gap-3 pt-4 text-label text-black/55">
...
function USArticleDetailPage() {
  const { slug } = (Route.useParams as any)() as { slug: string };
  const article = getHelpCenterArticle(slug, US_ARTICLES);
    ...
    >
      ...
      <article>
        ...
        <header className="mx-auto max-w-[760px] px-4 pb-8 pt-10 sm:px-6 lg:px-0">
          ...
          <h1 className="mt-4 text-black">{article.title}</h1>
          <p className="mt-4 font-sans text-lead text-black/60">{article.excerpt}</p>
          <div className="mt-6 flex flex-wrap items-center gap-4 border-t border-black/[0.07] pt-6">
            ...
            <div className="min-w-0">
              ...
              <p className="font-sans text-label text-black/55">{article.author.role}</p>
            </div>
      ...
      {related.length > 0 && (
        <section className="bg-white py-16 sm:py-20">
          <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
            <h2 className="text-h3 text-black">More guides</h2>
